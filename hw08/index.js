...
routes.add('GET', '/user', (request, response) => {
    response.setHeader('Content-Type', mime.lookup('json'));
...
routes.add('GET', '/date_time_qr_code_gif', async (request, response) => {
    response.setHeader('Content-Type', mime.lookup('gif'));
    const width = 1000;
    const qrCodeOptions = { errorCorrectionLevel: 'high' };
    const encoder = new GifEncoder(width, width);
    encoder.pipe(response);
    encoder.writeHeader();

    const getDateTimeQrCodePixels = () => {
        const dateTimeString = new Date().toISOString();
        const { modules } = QRCode.create(dateTimeString, qrCodeOptions);
        const { data, size } = modules;
        const scale = Math.trunc(width / (size + 2));
        const pixels = new Array(width * width * 4/* rgba */);
    ...
    encoder.addFrame(getDateTimeQrCodePixels());
    const intervalId = setInterval(
        () => encoder.addFrame(getDateTimeQrCodePixels()),
        1 * 1000 /* 1 sec */
    );
